...
public:
    ...
    {
        // Pulse timing must remain at audio rate: PulseIn1RisingEdge() lasts
        // one sample. The less urgent panel work below runs at 6 kHz.
        UpdateClock();
        const bool pressed = SwitchVal() == Switch::Down;
        if (pressed && !wasPressed_) pendingPress_ = true;
        wasPressed_ = pressed;

        if (controlDivider_ == 0) {
            // Main stands in for Bib's slider: divide its travel into four
            // clear regions. Up is a latching secondary layer for Delay and
            // Reverb. Knob/CV reads remain inside ProcessSample as required
            // by ComputerCard, but their mapped parameters need not update at
            // the full audio rate.
            const int32_t main = KnobVal(Knob::Main);
            mode_ = static_cast<uint8_t>((main * 4) >> 12);
            detailPage_ = SwitchVal() == Switch::Up && (mode_ == 1 || mode_ == 2);
            const bool tapePage = detailPage_ && mode_ == 1;
            const bool reverbPage = detailPage_ && mode_ == 2;
            pickup_.Select(mode_, detailPage_);
            // Pickup follows the physical pot; bipolar CV is added afterwards
            // so patching modulation never steals a stored pot position.
            ...
            UpdateControls(mode_, x, y, pressed, pendingPress_, tapePage, reverbPage);
            UpdateLeds(mode_, x, y, tapePage, reverbPage);
            pendingPress_ = false;
        }
        controlDivider_ = static_cast<uint8_t>((controlDivider_ + 1) & (kControlInterval - 1));

        // Freeze is intentionally still immediate. Its switch state is cheap
        // to follow per sample and should not wait for the next control tick.
        if (mode_ == 3 && !detailPage_) freeze_ = pressed;
...
private:
    static constexpr uint8_t kControlInterval = 8;
    ...
    uint8_t mode_ = 0;
    bool detailPage_ = false;
    bool pendingPress_ = false;

    void UpdateControls(uint8_t mode, int32_t x, int32_t y, bool pressed, bool newPress,
