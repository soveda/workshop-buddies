// Workshop block-audio control snapshot experiment.
//
...
// This first DSP pass deliberately fixes the profile. It has a modest audible
// delay/reverb path while avoiding the extreme settings until the exact engine
// and transport have been proven together on Workshop hardware.
constexpr int kBibDrive = 2048;
constexpr int kBibDelaySend = 2048;
constexpr int kBibDelayTime = 16384;
constexpr int kBibDelayFeedback = 2500;
constexpr int kBibReverbSend = 1000;
constexpr int kBibReverbFeedback = 2600;
constexpr int kBibShimmer = 0;
constexpr int kBibWetDryMix = 0;
constexpr int kBibOutputLevel = 2048;
...
{
    pwm_set_gpio_level(kLedPins[index], static_cast<uint32_t>(brightness) << 4);
}
...
{
    // LEDs 0/1 remain dedicated to transport failures. In Middle, LEDs 3-5
    // show Main/X/Y. Up and Down replace that display with a clear switch
    // indicator. Pulse 1 briefly lights LED 2; it is otherwise dark so each
    // rising edge is unambiguous during this diagnostic test.
    if (controls.switchPosition == SwitchPosition::Down)
    {
        SetLed(3, true);
        SetLed(4, false);
        SetLed(5, false);
    }
    else if (controls.switchPosition == SwitchPosition::Up)
    {
        SetLed(3, false);
        SetLed(4, true);
        SetLed(5, false);
    ...
    {
        SetLedBrightness(3, static_cast<uint16_t>(controls.knobs[0]));
        SetLedBrightness(4, static_cast<uint16_t>(controls.knobs[1]));
        SetLedBrightness(5, static_cast<uint16_t>(controls.knobs[2]));
    }

    if (pulseFlashBlocks != 0)
    {
        SetLed(2, true);
        --pulseFlashBlocks;
    }
...
void ProcessBlock(const StereoBlock &input, const ControlSnapshot &controls, StereoBlock &output)
{
    ...
    process_bib(reinterpret_cast<int16_t *>(output.frames), kBibDrive, kBibDelaySend, kBibDelayTime, kBibDelayFeedback,
                kBibReverbSend, kBibReverbFeedback, kBibShimmer, kBibWetDryMix, kBibOutputLevel, 0, 0, 0, 0, 0);
}
