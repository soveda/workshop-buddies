// Workshop block-audio original Bib guard, wavefold-cap, and shift-pickup experiment.
//
...
void UpdatePickedControl(uint8_t mode, uint8_t index, int control)
{
    ...
    const int separation = control - stored;
    ...
    // Division-free relative pickup. A moved physical knob adjusts the saved
    // value by its own movement plus a gentle pull toward its position.
    stored = ClampControl(stored + delta + (separation >> 4));
}
