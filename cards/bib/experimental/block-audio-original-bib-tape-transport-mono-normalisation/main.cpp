...
void __isr __not_in_flash_func(AudioDmaComplete)()
{
    ...
    // Match Bibesque's musical mono default: Audio In 1 remains centred when
    // Audio In 2 is unpatched, while a real stereo patch stays independent.
    if (!connected[1]) inputRight = connected[0] ? inputLeft : 0;
