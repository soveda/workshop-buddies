// Workshop block-audio original Bib knob-pickup experiment.
//
...
void UpdatePickedControl(uint8_t mode, uint8_t index, int control)
{
    ...
    // Bib's software-knob catch-up response, rescaled from q24 to Workshop's
    // 12-bit controls. A parked knob therefore cannot cause a value jump.
...
bool UpdateSwitchDebounce(SwitchPosition requested)
{
    ...
    return bibSwitchState.stable == SwitchPosition::Down;
}
...
BibParameters UpdateBibParameters(const ControlSnapshot &controls)
{
    ...
    {
        activePage = mode;
        PrimePagePickup(mode, xControl, yControl);
    }
    ...
    if (mode == 3 && bibSwitchState.stable == SwitchPosition::Down)
    {
...
void ProcessBlock(const StereoBlock &input, const ControlSnapshot &controls, StereoBlock &output)
{
    ...
    process_bib(reinterpret_cast<int16_t *>(output.frames), parameters.drive, parameters.delaySend, parameters.delayTime,
