// Workshop block-audio original Bib shimmer experiment.
//
...
void UpdateShimmerControl(int control)
{
    ...
    bibSwitchState.shimmerAmount = ClampControl(bibSwitchState.shimmerAmount + delta);
}
...
BibParameters UpdateBibParameters(const ControlSnapshot &controls)
{
    ...
    const uint8_t controlPage = shimmerPage ? 4 : mode;
    ...
    const int xValue = ClampControl(pageValues[mode][0] + xCv);
    const int yValue = ClampControl(pageValues[mode][1] + yCv);
    ...
    BibParameters parameters = {
        ...
        ClampControl(bibSwitchState.shimmerAmount + xCv),
    };
