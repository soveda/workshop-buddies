...
void __isr __not_in_flash_func(AudioDmaComplete)()
{
    ...
    // Keep a mono source centred without doubling its power inside the stereo
    // processor. This also keeps the input's analogue noise floor unchanged.
    if (!connected[1] && connected[0])
    {
        inputLeft = (inputLeft * 2896) >> 12; // approximately -3 dB
