...
channels before the original stereo Bib DSP at equal-power level: each channel
is reduced by approximately 3 dB so the centred mono signal does not double
its power or noise floor. A second patched input remains a true independent
right channel. With neither input patched, both channels are silent and the
normalisation probe cannot become audible noise.
