# Buzzrito Original Wobble Half-Depth Test Protocol

The modal WebUSB, glide, BOC, and noise paths are inherited from passed
versions. This protocol tests only the increased original per-saw
`wobble_amount` cap.

## Test

1. Flash `cards/buzzrito/test-artifacts/workshop_buzzrito_original_wobble_half_test.uf2`.
...
6. Leave `wobble_speed` at the preset value. Re-enter editor mode, set that
   ...
   controls stationary, listen for stronger but still restrained independent
   saw movement.
