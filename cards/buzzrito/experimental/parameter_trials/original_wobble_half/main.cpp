...
private:
                                                   ...
                                                   int32_t wobble_speed)
    {
        ...
        const int32_t bounded_wobble_amount = clampi(wobble_amount, 0, 4096) >> 1;
        const int32_t bounded_wobble_speed = clampi(wobble_speed, 0, 4096);
        boc_wobble_q19_ = (update_interp_noise(boc_noise_, boc_pink_, bounded_wobble_speed) *
                            bounded_boc_amount) >> 10;
        ...
        for (int i = 0; i < kNumSaws; ++i)
        {
            ...
            const int32_t wobble = (update_interp_noise(saw_wobble_noise_[i], saw_wobble_pink_[i],
                                                         bounded_wobble_speed) * bounded_wobble_amount) >> 9;
