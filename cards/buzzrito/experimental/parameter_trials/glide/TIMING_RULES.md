...
- Glide is calculated every 64 samples, then its `ddelta_t` is applied once per
  rendered sample. This trial uses that rule directly.
- A future BOC interpolation-noise update must receive its original
  `wobble_speed` once per 64 samples, or receive `wobble_speed / 64` per sample
  with an equivalent fractional accumulator.
