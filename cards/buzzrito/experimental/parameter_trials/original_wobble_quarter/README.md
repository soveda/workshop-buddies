# Buzzrito Original Wobble Trial

This isolated test derives from the selected quarter-depth noise and full-depth
BOC version. It replaces only the simplified sine wobble with the original
per-saw interpolation-noise wobble algorithm.

The inherited glide, full-depth BOC, and quarter-depth noise behavior has
passed hardware testing. This original-wobble trial requires testing.
...
This first trial caps `wobble_amount` at one quarter of the original range. At
...
```sh
cmake -S . -B /private/tmp/workshop-buzzrito-original-wobble -DCMAKE_BUILD_TYPE=Release
cmake --build /private/tmp/workshop-buzzrito-original-wobble -j2
```
