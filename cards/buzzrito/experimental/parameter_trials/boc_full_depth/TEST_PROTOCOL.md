# Buzzrito Full-Depth BOC Test Protocol

The modal WebUSB, glide, BOC, and noise paths are inherited from passed
versions. This protocol tests only the original full `boc_amount` range.

## Test

**Result: passed.** Full-depth BOC is audible and musical without card lockup.

1. Flash `cards/buzzrito/test-artifacts/workshop_buzzrito_boc_full_depth_test.uf2`.
...
6. Re-enter editor mode, set that same preset's `boc_amount` to maximum, save,
   ...
   to the half-depth test: it should be approximately twice as strong while
   retaining continuous, random movement.
