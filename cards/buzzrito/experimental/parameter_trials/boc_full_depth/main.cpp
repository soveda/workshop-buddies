...
private:
    ...
    void __not_in_flash_func(update_controls)()
    {
        ...
        // The first audio-noise trial deliberately uses one sixteenth of the
        // original mix range. A zero source value takes a complete bypass.
        noise_level_ = clampi(preset.noise_level, 0, 4096) >> 4;
    ...
    static int32_t __not_in_flash_func(update_boc_interp_noise)(InterpNoise &noise, PinkNoise &pink,
                                                                  int32_t speed)
    {
        noise.time += speed;
    ...
    void __not_in_flash_func(update_pitch_deltas)(int32_t pitch_mv, int32_t spread, int32_t glide,
                                                   int32_t boc_amount, int32_t boc_speed)
    {
        // The original engine calculates glide once per 64-sample I2S block,
        // then applies the resulting delta increment for each sample. Keep
        // that cadence here rather than treating a block-rate coefficient as
        // a per-sample value.
        if (pitch_update_counter_++ != kOriginalBlockSize - 1)
        {
            return;
        }
        pitch_update_counter_ = 0;

        // BOC originally advances once for each 64-sample DSP block. This
        // calibration trial restores its full original depth and preserves
        // its shared sub/comb and half-depth saw routing.
        const int32_t bounded_boc_amount = clampi(boc_amount, 0, 4096);
        boc_wobble_q19_ = (update_boc_interp_noise(boc_noise_, boc_pink_,
                                                    clampi(boc_speed, 0, 4096)) * bounded_boc_amount) >> 10;

        const static int middle_c_offset_q19 = (int)(23.4806373824f * (1 << 19));
        const int32_t base_log_q19 = pitch_to_log_q19(pitch_mv) + middle_c_offset_q19 + boc_wobble_q19_;
        const uint32_t sub_target = exp2_table(base_log_q19 - (1 << 19));
        ...
        for (int i = 0; i < kNumSaws; ++i)
        {
            ...
            wobble_phase_[i] += increment * kOriginalBlockSize;
            ...
            saw_ddelta_[i] = calc_ddelta_t(static_cast<int32_t>(saw_delta_[i]),
                                            static_cast<int32_t>(target), glide_shape);
